...
app.service("ProposicionesService", ["$q", "$timeout","$http",
  function ProposicionesService($q, $timeout,$http) {
...
app.controller("ProposicionesCtrl", ["$scope", "$uibModal", "ProposicionesService", "DTOptionsBuilder", "DTColumnBuilder" ,"$stateParams",
  function ProposicionesCtrl($scope, mo, ProposicionesService,DTOptionsBuilder, DTColumnBuilder,$stateParams) {
	  $scope.comisionId =$stateParams.comision;
	    
	    $scope.url1='/SilPortal/REST/mysql/getIniciativaPropByComision/'+$scope.comisionId+"/19";
	    ProposicionesService.get($scope.url1).then(function(respons) {
		...
		ProposicionesService.get($scope.url2).then(function(respons) {

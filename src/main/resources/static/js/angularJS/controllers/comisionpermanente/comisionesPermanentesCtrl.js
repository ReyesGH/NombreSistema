...
app.service("ComisionesService", ["$q", "$timeout","$http",
  function ComisionesService($q, $timeout,$http) {
	  this.getPartidos = function getPartidos() {
		    return $http({
		      method: 'GET',
		      // url: '/REST/partidos/getpartidos/2'
		       url: '/SilPortal/REST/partidos/getpartidos/3'
		    });
		  };
...
app.controller("ComisionesPermanentesCtrl", ["$scope","$rootScope", "$uibModal", "ComisionesService", "DTOptionsBuilder", "DTColumnBuilder" ,
 function ComisionesPermanentesCtrl($scope,$rootScope, mo, ComisionesService,DTOptionsBuilder, DTColumnBuilder) {
    ...
    // dt.someClickHandler = someClickHandler;
     dt.persons = {};
      dt.options = DTOptionsBuilder
        // .fromSource("/REST/Legislador/getLegisladores/2")
         .fromSource("/SilPortal/REST/Legislador/getLegisladores/5")
        ...
        .withLanguage({
          ...
          search: "_INPUT_",
          searchPlaceholder: "Buscar Integrantes Comisión",
     ...
     function actionsHtml1(info, type, full, meta) {
       ...
       // function actionsHtml(info, type, full, meta) {
         // dt.persons[info.id] = info;
          // return '<img src="images/diputados/AnaLaura.jpg" alt=" "
			// ng-click="showCase.edit(showCase.persons[' + info.id + '])">' ;
      // }
